...
IMAGES_PATH = os.path.sep.join(
	[DATASET_PATH, "ISIC2018_Task1-2_Training_Input"])
MASKS_PATH = os.path.sep.join(
	[DATASET_PATH, "ISIC2018_Task1_Training_GroundTruth"])
...
class LesionBoundaryDataset(utils.Dataset):
	...
	def load_mask(self, imageID):
		...
		masks = np.zeros((
			annotMask.shape[0], annotMask.shape[1], 1),	dtype="uint8")
...
if __name__ == "__main__":
	...
	ap.add_argument(
		"-m", "--mode", required=True, help="either 'train', 'predict',\
		or 'investigate'")
	ap.add_argument("-w", "--weights", help="optional path to\
	pretrained weights")
	ap.add_argument("-i", "--image", help="optional path to input\
	image to segment")
	...
	# check to see if we are predicting using a trained Mask R-CNN
		...
		for i in range(0, r["rois"].shape[0]):
			...
			image = visualize.apply_mask(
				image, mask, (1.0, 0.0, 0.0), alpha=0.5)
	...
	# check to see if we are investigating our images and masks
		...
		for i in np.random.choice(trainDataset.image_ids, 3):
			...
			visualize.display_top_masks(
				image, masks, classIDs,	trainDataset.class_names)
